<template>
  <div class="container-fluid px-4 py-4">
    ...
    <div v-if="showEditModal" class="modal-backdrop d-flex align-items-center justify-content-center">
      <div class="modal-content shadow-lg rounded-4 overflow-hidden" style="max-width: 600px">
        ...
        <div class="modal-body px-4 py-3">
          ...
          <div class="form-floating mb-3">
            <input type="text" id="nombre-indicador" v-model="indicadorEditForm.nombreIndicador"
              class="form-control shadow-sm" placeholder="Nombre del indicador" required />
            <label for="nombre-indicador"><i class="bi bi-tag me-2 text-primary"></i>Indicador
              <span class="text-danger">*</span></label>
          </div>

          <!-- Proyecto del indicador -->
          <div class="form-floating mb-3">
            <select id="proyecto" v-model="indicadorEditForm._idProyecto" class="form-select shadow-sm" required>
              <option value="1.1.2">1.1.2</option>
              <option value="1.1.3">1.1.3</option>
              <option value="1.1.4">1.1.4</option>
            </select>
            <label for="proyecto"><i class="bi bi-folder me-2 text-primary"></i>Proyecto
              <span class="text-danger">*</span></label>
          </div>
          ...
          <div class="row g-3">
            ...
            <div class="col-md-6">
              <div class="form-floating">
                <input type="number" id="numero" v-model="indicadorEditForm.numero" class="form-control shadow-sm"
                  placeholder="0" />
            ...
            <div class="col-md-6">
              <div class="form-floating">
                <input type="number" id="numerador" v-model="indicadorEditForm.numerador" class="form-control shadow-sm"
                  placeholder="0" />
                <label for="numerador"><i class="bi bi-calculator me-2 text-primary"></i>Numerador</label>
              </div>
            </div>
          </div>
        </div>
    ...
    <div v-if="showConfigModal" class="modal-backdrop d-flex align-items-center justify-content-center">
      <div class="modal-content shadow-lg rounded-4 overflow-hidden" style="max-width: 600px">
        <!-- Cabecera -->
        <div class="modal-header bg-gradient-primary text-white py-3 px-4">
          <h3 class="modal-title m-0"><i class="bi bi-gear me-2"></i>Configurar Indicador</h3>
          <button @click="closeConfigModal" class="btn-close btn-close-white" aria-label="Close"></button>
        </div>
        ...
        <!-- Pie -->
        <div class="modal-footer bg-light d-flex justify-content-end border-0 px-4 py-3">
          <button @click="closeConfigModal" class="btn btn-outline-secondary rounded-pill px-4 me-2">
            <i class="bi bi-x-circle me-2"></i>Cancelar
          </button>
    ...
    <div class="card shadow-lg border-0 rounded-4 overflow-hidden">
      <div class="card-header bg-gradient-primary-to-secondary text-white p-4 border-0">
        <div class="d-flex justify-content-between align-items-center">
          ...
          <span class="badge bg-white text-primary rounded-pill px-3 py-2 d-flex align-items-center">
      ...
      <div class="card-body p-0">
        <div class="table-responsive">
          <table class="table table-hover align-middle mb-0 table-striped">
            ...
            <tbody>
              <tr v-for="(indicador, index) in paginatedData" :key="indicador._id" class="border-bottom">
                <td class="ps-4 py-3">
                ...
                <td class="py-3">
                  <span class="badge bg-secondary text-white rounded-circle p-2 fw-bold"
                ...
                <td class="py-3">
                  <button class="btn btn-sm btn-outline-primary rounded-pill px-3" @click="configurarIndicador()">
                    <i class="bi bi-pencil-square me-1"></i>configurar
                  </button>
                </td>
                <td class="py-3">
                  <button class="btn btn-sm btn-outline-primary rounded-pill px-3" @click="editarIndicador(indicador)">
                    <i class="bi bi-pencil-square me-1"></i>Editar
                  </button>
                </td>
                <td class="py-3">
                  <button class="btn btn-sm btn-outline-danger rounded-pill px-3" @click="eliminarIndicador(indicador)">
          ...
          <nav aria-label="Paginación">
            <ul class="pagination pagination-md justify-content-center mb-0">
              <li class="page-item me-1" :class="{ disabled: currentPage === 1 }">
                <button class="page-link border-0 rounded-circle shadow-sm" @click="prevPage"
                  :disabled="currentPage === 1" aria-label="Anterior">
                  <i class="bi bi-chevron-left"></i>
                </button>
              </li>
              <li class="page-item mx-1" v-for="page in pages" :key="page" :class="{ active: page === currentPage }">
                <button class="page-link border-0 rounded-circle shadow-sm fw-medium" @click="goToPage(page)">
                  {{ page }}
                </button>
              </li>
              <li class="page-item ms-1" :class="{ disabled: currentPage === totalPages }">
                <button class="page-link border-0 rounded-circle shadow-sm" @click="nextPage"
                  :disabled="currentPage === totalPages" aria-label="Siguiente">
