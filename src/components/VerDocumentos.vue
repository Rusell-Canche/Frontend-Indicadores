          ...
          <div class="modal-body">
            <form @submit.prevent="submitEdit">
              <div v-for="campo in camposDocumento" :key="campo" class="mb-3">
                ...
                />
                <input
                  v-else-if="campo !== 'Recurso Digital'"
                  type="text"
                  class="form-control"
                  v-model="documentoEdit[campo]"
                  :id="campo"
              ...
              <div
                v-if="documentoEdit['Recurso Digital'] && documentoEdit['Recurso Digital'].length"
              >
                <h6>Archivos Adjuntos:</h6>
                <div class="d-flex flex-wrap">
                  <div
                    class="file-preview"
                    v-for="(file, index) in documentoEdit['Recurso Digital']"
                    :key="index"
                  ...
                  >
                    ...
                    <template v-else-if="isPDF(file)">
                      <a :href="getFileUrl(file)" target="_blank" class="btn btn-sm btn-secondary"
                        >Ver PDF</a
                      >
                      <button
                        type="button"
      ...
      if (selectedColeccion.value) {
        ...
        try {
          ...
          const response = await axios.get(
            `http://127.0.0.1:8000/api/documentos/${selectedColeccion.value.id}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
          ...
          if (documentos.value.length > 0) {
            ...
            camposDocumento.value = Object.keys(documentoConMasCampos).filter(
              (campo) => campo != '_id',
            )
          } else {
      ...
      try {
        ...
        if (result.isConfirmed) {
          ...
          const response = await axios.delete(
            `http://127.0.0.1:8000/api/documentos/${selectedColeccion.value.nombre_coleccion}/${documentoId}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
        ...
        Swal.fire(
          'Error',
          'Hubo un problema al eliminar el documento. Por favor, inténtalo de nuevo.',
          'error',
        )
      }
    }
      ...
      try {
        ...
        Object.keys(documentoEdit.value).forEach((key) => {
          if (key !== 'Recurso Digital' || key !== '_id') {
            formData.append(`document_data[${key}]`, documentoEdit.value[key])
          }
        })

        // Remover el campo '_id' del formData
        formData.delete('document_data[_id]')
        ...
        const token = localStorage.getItem('apiToken')
        await axios.post(
          `http://127.0.0.1:8000/api/documentos/${selectedColeccion.value.nombre_coleccion}/${documentoEdit.value._id.$oid}`,
          formData,
          {
