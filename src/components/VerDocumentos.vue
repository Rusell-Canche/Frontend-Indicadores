                  ...
                  <div v-if="documento[campo] && documento[campo].length">
                    ...
                    <template v-if="isImage(documento[campo][0])">
                      <a href="#" @click.prevent="openFancybox(documento, documento[campo])">
                        ...
                        <img src="../assets/8344917.png" class="card-img-top thumbnail" alt="Pdf"
  ...
  setup() {
    ...
    const onColeccionSelected = async () => {
      if (selectedColeccion.value) {
        ...
        try {
          const response = await axios.get(`/documentos/${selectedColeccion.value.id}`);
    ...
    const eliminarDocumento = async (documentoId) => {
      try {
        ...
        if (result.isConfirmed) {
          const response =  await axios.delete(`/documentos/${selectedColeccion.value.nombre_coleccion}/${documentoId}`);
    ...
    const submitEdit = async () => {
      try {
        ...
        await axios.post(`/documentos/${selectedColeccion.value.nombre_coleccion}/${documentoEdit.value._id.$oid}`, formData, {
